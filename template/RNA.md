...
9. **非空转自证**：断言、闸门、校验器的价值**不在它存在，而在它能红**。写或改一处判据时，必须证明它**抓得住它声称要抓的东西**：
   - **变异注入**（首选，最硬的证据）：故意弄坏被测机制 ⇒ 判据必须变红；还原 ⇒ 必须转绿。两侧都要做，只测一侧会放过"永远红的坏判据"。
   - **结构性自检**（代价高时的替代，且**每次运行都做**）：把校验器跑在**合成的坏输入**上，该报的必须报；再跑在**合成的好输入**上，不该报的必须不报。只测坏输入会放过**过宽的**校验器——而**没人信的检查比没有检查更糟**。
   - **不变量还要数它覆盖了几态**：只覆盖两态却声称三态的判据，是**披着绿衣的空转**。
   - **按代价分级**：安全、财务、不可逆操作的判据**必须**自证；纯样式与一次性脚本可只做结构性自检。**节能的前提是判据仍在承重处生效**，不是把该证的省掉。
