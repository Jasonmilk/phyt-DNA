...
| 9 | **非空转自证** | 断言、闸门、校验器的价值**不在它存在，而在它能红**。写或改判据时必须证明它抓得住声称要抓的东西：**变异注入**（弄坏被测机制 ⇒ 必须变红；还原 ⇒ 必须转绿）、或**双向结构性自检**（合成坏输入必须报、合成好输入必须不报）、并**数清不变量覆盖了几态**。按代价分级：安全/不可逆操作的判据**必须**自证 |

> **项目特有铁律**不占通用编号 —— 每个项目在 `DNA.md` 第五节定义自己的（如凭证红线、沙箱边界）。
...
3. **RNA.md** — 确认三层加载协议，确认 AI 协作铁律（通用 9 条 + 项目特有），确认大版本更新 SOP
...
**测试通过不等于做完。** 跑完 `npm test` 之类的测试、类型检查与构建之后，
**还要亲手走一遍受影响的那条真实流程**。

这是「反向证据」的另一半：变异测试证明**断言会失败**，但证明不了**功能能用**。
两者都需要：

| 问题 | 证据 |
|---|---|
| 这条断言会不会失败？ | 变异 —— 把代码改坏，断言必须转红 |
| 这个功能能不能用？ | 走真实流程 —— 打开它、用它、看它 |

曾有一次实现通过了四套绿灯，而**没有任何人打开过它改的那个界面**。
测试是对的，功能是未验证的。

## 贡献与范围

phyt-DNA 是**通用**方法论。正是范围边界保证了它可被复用：

- **范围内** —— 任何项目都能套用的机制：文档形态、阶段流转 SOP、ADR 规范、加载协议、
  防腐化铁律、保护原则。
- **范围外** —— 只对某一个项目或某一个生态成立的东西：项目名、仓库路径、落地状态、
  路线图、缺陷登记表、跨仓台账。

两条推论，都是刻意的：

1. **举例按角色，不点名。** "一个记忆中枢项目"在范围内；写出它的名字则不在。读者需要
   知道的是**哪一类**项目验证过这条规则，不是**哪一个**。
2. **生态自身的状态归生态自己的仓。** 一个事实一个来源。如果某个项目完成一次迁移就
   必须回来改方法论文档，耦合已经在那里了——改动只是症状，不是修复。

只对某一个具体项目成立的改动，属于那个项目——哪怕它是个好改动。
...
phyt-DNA 方法论从真实项目实践中提炼、验证、闭环。采用项目一律**按角色指代，不点名**——
本仓刻意保持通用（见[贡献与范围](#贡献与范围)）：

| 项目角色 | 对方法论的贡献 |
|---|---|
| 一个记忆 / 认知中枢项目 | 发源地——方法论起源，早期阶段实践 |
| 一个执行躯体项目 | 迁移参考——完整迁移，语言重构验证 |
| 一个信息采集项目 | 最新迁移——完整迁移 + 闭环补全 |
| 一组协议家族实践 | "一个权威，多种呈现"架构验证 |

**关键闭环补全历史：**
- v1.0 (2026-08-29): 从三个早期采用项目的实践中提炼，整合为独立方法论锚点项目。补全 PLAN.md 阶段流转 SOP（此前 SOP 只要求写 GROWTH/ADR/DEPRECATE，未明确要求 PLAN 阶段流转，导致知识腐化风险）。补全 ADR 命名规范（文件名与引用名必须一致）。
